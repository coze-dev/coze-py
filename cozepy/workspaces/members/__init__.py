...
class WorkspacesMembersClient(object):
    ...
    def delete(self, *, workspace_id: str, user_ids: List[str], **kwargs) -> DeleteWorkspaceMemberResp:
        ...
        批量移除空间中的用户。 接口描述 调用本 API 将用户从指定的工作空间中移除。只能移除空间管理员或成员，不支持移除空间所有者。批量移除用户时，你可以在返回结果中查看移除失败的用户以及具体原因。

        :param workspace_id: 需要移除用户的空间 ID。 进入指定空间，空间页面 URL 中 `space` 参数后的数字就是 Space ID。例如`https://www.coze.cn/space/736163827687053****`，Space ID 为`736163827687053****`。
        :param user_ids: 要移除的成员，数量最多5
        """
        url = f"{self._base_url}/v1/workspaces/{workspace_id}/members"
        headers: Optional[dict] = kwargs.get("headers")
        body = remove_none_values(
