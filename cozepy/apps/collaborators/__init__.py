...
class AppsCollaboratorsClient(object):
    ...
    def create(self, *, app_id: str, collaborators: List[AppCollaborator], **kwargs) -> AddAppCollaboratorResp:
        ...
        添加低代码应用的协作者。 接口限制 套餐限制：扣子企业版（企业标准版、企业旗舰版）。 每次请求只能添加一位协作者。如需添加多位，请依次发送请求。 协作者只能是该工作空间的成员。 不支持渠道类型 OAuth 应用。使用 OAuth JWT 应用和服务访问令牌时，只需要有对应权限点即可。其余认证方式，只有 应用的所有者和协作者 能添加协作者。 主账号内的所有子账号共享同一 API 的流控额度，单个 API 的流控限制为 5 QPS。
    ...
    def delete(self, *, app_id: str, user_id: str, **kwargs) -> RemoveAppCollaboratorResp:
        ...
        删除扣子应用的协作者。 删除协作者时，扣子会将该协作者创建的工作流、插件等资源转移给应用的所有者。 接口限制 主账号内的所有子账号共享同一 API 的流控额度，单个 API 的流控限制为 5 QPS。 每次请求只能删除一位协作者。如需删除多位，请依次发送请求。 使用个人访问令牌时，只有应用的所有者和协作者有权删除。使用 OAuth 应用和服务访问令牌时，只需要有对应权限点即可。
