...
class ConversationsClient(object):
    ...
    def create(
        ...
        **kwargs,
        ...
        创建一个会话。 接口描述 会话是智能体和用户之间的基于一个或多个主题的问答交互，一个会话包含一条或多条消息。创建会话时，扣子会同时在会话中创建一个空白的上下文片段，用于存储某个主题的消息。后续发起对话时，上下文片段中的消息是模型可见的消息历史。 你可以在创建会话时同步写入消息，或者创建会话后另外调用 创建消息 API 写入消息。消息默认写入到最新的一个上下文片段中，对话时将作为上下文传递给模型。 会话、对话、消息和上下文片段的概念说明，可参考 基础概念 。 如果需要将不同业务侧用户的会话互相隔离，请参见 如何实现会话隔离 。
    ...
    def retrieve(self, *, conversation_id: str, **kwargs) -> Conversation:
        ...
        通过会话 ID 查看会话信息。 接口限制 仅支持查询本人创建的会话。
    ...
    def update(self, *, conversation_id: str, name: Optional[str] = None, **kwargs) -> Conversation:
        ...
        会话创建者可以更新指定会话的会话名，以便更好地识别和管理会话。 在创建会话时，扣子会默认将用户发送的第一个消息内容作为会话名称。用户也可以根据会话的实际内容或主题对会话名称进行更新，从而更直观地区分不同的会话，提升管理效率。
    ...
    def list(
        ...
        **kwargs,
        ...
        调用此 API 之前，应确认当前使用的访问密钥拥有智能体所在工作空间的权限。 仅支持通过此 API 查看智能体在 API 或 Chat SDK 渠道产生的会话。 仅支持查询本人创建的会话。 查看指定智能体的会话列表。
...
class AsyncConversationsClient(object):
    ...
    async def list(
        ...
        **kwargs,
        ...
        查看指定智能体的会话列表。 调用此 API 之前，应确认当前使用的访问密钥拥有智能体所在工作空间的权限。 仅支持通过此 API 查看智能体在 API 或 Chat SDK 渠道产生的会话。 仅支持查询本人创建的会话。
