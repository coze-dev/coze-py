...
class DocumentsClient(object):
    ...
    def create(
        ...
        **kwargs,
        ...
        接口说明 通过此 API，你可以向文本或图片知识库中上传文件。
        本 API 仅适用于扣子知识库的文件上传操作，不适用于火山知识库上传文件。
        火山知识库上传文件请参见 火山知识库上传文件 。
        上传图片到图片知识库时，可以通过 caption_type 参数设置系统标注或手动标注，如果选择手动标注，还需要调用 更新知识库图片描述 API 手动设置标注。
        支持的上传方式如下： 注意事项 每次最多可上传 10 个文件。
        必须上传和知识库类型匹配的文件，例如 txt 等文档类型的文件只能上传到文档知识库中，不可上传到表格知识库中。
        每个请求只能选择一种上传方式，不支持同时上传在线网页和本地文档。
        仅知识库的所有者可以向知识库中上传文件。
        文本知识库 通过 Base 64 上传本地文件 上传在线网页 ✅ ❌ ❌ 通过 上传文件 上传 ❌ 上传方式 图片知识库 ✅ 格式支持 pdf、txt、doc、docx 类型。
        ✅
...
class AsyncDocumentsClient(object):
    ...
    async def create(
        ...
        **kwargs,
        ...
        """
        创建知识库文件
        ❌ 文本知识库 ✅ 格式支持 pdf、txt、doc、docx 类型。
        ❌ 调用此 API 向指定的扣子知识库上传文件。
        接口说明 通过此 API，你可以向文本或图片知识库中上传文件。
        本 API 仅适用于扣子知识库的文件上传操作，不适用于火山知识库上传文件。
        火山知识库上传文件请参见 火山知识库上传文件 。
        ...
        每个请求只能选择一种上传方式，不支持同时上传在线网页和本地文档。
        仅知识库的所有者可以向知识库中上传文件。
        上传方式 ✅ 图片知识库 通过 Base 64 上传本地文件 通过 上传文件 上传 上传在线网页 ✅ ❌
        :param document_bases: 表格类型一次只能创建一个待创建的文档信息
        """
        url = f"{self._base_url}/open_api/knowledge/document/create"
        headers = {"Agw-Js-Conv": "str"}
