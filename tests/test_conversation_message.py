...
def test_conversation_message():
    ...
    # create conversation
    conversation = cli.conversations.v1.create(
        messages=[
    ...
    conversation_retrieve = cli.conversations.v1.retrieve(conversation_id=conversation.id)
    assert conversation.id == conversation_retrieve.id

    # create message
    user_input = Message.user_text_message("nice to meet you.")
    message = cli.conversations.v1.message.create(
    ...
    # retrieve message
    message_retrieve = cli.conversations.v1.message.retrieve(conversation_id=conversation.id, message_id=message.id)
    assert message_retrieve is not None
    assert message.id == message_retrieve.id

    # list message
    message_list = cli.conversations.v1.message.list(conversation_id=conversation.id, message_id=message.id)
    assert len(message_list) > 2

    # update message
    cli.conversations.v1.message.update(
        conversation_id=conversation.id,
    ...
    # delete message
    cli.conversations.v1.message.delete(
        conversation_id=conversation.id,
        message_id=message.id,
    )
